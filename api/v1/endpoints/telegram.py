...
class TelegramApi:
    ...
    async def telegram_participants(chat_id:int, recents_users:bool, save_users:bool):
        try:
            ...
            all_participants = await telegram._all_partifipants(recents_users=recents_users, save_users=save_users, group=group)
    ...
    async def ban_user(chat_id:int, user:Union[str, int], time_remove:bool, days:int = None):
        '''
        Id do grupo EX: -1001475740997 é adicionado -100 na frente do id, o id do usuario poder ser somente o username,\n
        time_remove: Verifica se pretende agendar a data de banimento do usuario,\n
        days:  dias de agendamento para o banimento
        '''

        if not isinstance(chat_id, int):
                raise HTTPException(detail=f'O campo chat_id deve ser numéros inteiros, EX: -1001495540997', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY)
        if time_remove:
            if not isinstance(days, int) or days == ' ':
                raise HTTPException(detail=f'O campo days deve ser numéros inteiros e não pode ser um campo vazio se estiver com time_remove habilitado', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY)
        else:
            days = 0
        ...
        try:
            ...
            raise HTTPException(detail=f'Ocorreu o seguinte erro: {e}', status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
