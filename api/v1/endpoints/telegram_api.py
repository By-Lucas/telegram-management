...
class TelegramEndpint:
    ...
    async def ban_user(chat_id: int, user: Union[str, int], time_remove: bool, days: int = None):
        ...
        if not isinstance(chat_id, int):
            raise HTTPException(detail=f'O campo chat_id deve ser numéros inteiros, EX: -1001495540997',
                                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY)
        if time_remove:
            if not isinstance(days, int) or days == ' ':
                raise HTTPException(
                    detail=f'O campo days deve ser numéros inteiros e não pode ser um campo vazio se estiver com time_remove habilitado',
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY)
        else:
            days = 0
